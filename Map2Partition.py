...
class Map_to_Partition():
    ...
    def __init__(self, qt_map, msbt_map, msdire_map, chroma_factor, lamb1=0.7, lamb2=0.7, lamb3=1.5, lamb4=0.3, lamb5=0.7, debug_mode=False):
        self.qt_map = qt_map
        ...
        self.out_bt_map = np.zeros((3, 16, 16), dtype=np.int8)
    ...
    def set_bt_partition_vector(self, x, y, h, w):
        ...
        self.out_bt_map[0, x:x+h, y:y+w] = best_node0.bt_map[x:x+h, y:y+w]
        self.out_bt_map[1, x:x+h, y:y+w] = best_node1.bt_map[x:x+h, y:y+w]
        self.out_bt_map[2, x:x+h, y:y+w] = best_node2.bt_map[x:x+h, y:y+w]
        # ************************* debug fal **************************
    ...
    def get_partition(self):
        ...
        if self.debug_mode:
            return self.out_bt_map, self.out_msdire_map
        else:
            return self.par_vec, self.out_msdire_map


def map_to_parititon(qt_map, bt_map, dire_map, chroma_factor, debug_mode=False):
    # start_time = time.time()
    partition = Map_to_Partition(qt_map, bt_map, dire_map, chroma_factor, debug_mode=debug_mode)
    p, d = partition.get_partition()
    # total_time = time.time() - start_time
    # debug_mode=True, 返回深度图和方向图
    if debug_mode:
        return p, d
    else:
